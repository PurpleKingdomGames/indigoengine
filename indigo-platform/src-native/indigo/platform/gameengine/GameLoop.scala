...
import indigoengine.shared.collections.mutable.Batch as MutableBatch
...
final class GameLoop[StartUpData, GameModel](
  ...
  private val frameDeltaRecord: MutableBatch[Double] = MutableBatch(0.0d, 0.0d, 0.0d, 0.0d, 0.0d)
  ...
  private val runner: (Double, Double, Double) => Unit =
    gameConfig.frameRateLimit match
      ...
      case Some(fps) =>
        (time, timeDelta, lastUpdateTime) =>
          frameDeltaRecord.drop(1)
          frameDeltaRecord.append(timeDelta)
