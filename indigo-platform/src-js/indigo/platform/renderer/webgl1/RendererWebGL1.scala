...
final class RendererWebGL1(
  ...
  def drawScene(sceneData: ProcessedSceneData, runningTime: Seconds): Unit = {
    ...
    sceneData.layers.foreach { layer =>
      ...
      drawLayer(layer.entities.toJSArray, standardShaderProgram, projection)
    }
    ...
    displayEntities
      ...
      .foreach {
        case (letters: DisplayTextLetters, _) =>
          renderEntities(letters.letters.toJSArray, shaderProgram, baseTransform)
          setBaseTransform

        case (group: DisplayGroup, _) =>
          renderEntities(group.entities.toJSArray, shaderProgram, group.transform)
          setBaseTransform
