...
object CompactLayers:
  ...
  /** Optimise the number of full screen render composites we need to perform by collapsing the layer tree structure
    ...
    *
    *   1. Layer Entries were merged together by key during scene composition, that is the purpose of layer keys. Here
    *      we discard layer keys as we no longer need them, and flatten layer entry groups where the rules allow, i.e.
