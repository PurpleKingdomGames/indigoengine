...
/** Events for capturing a snapshot of the rendered scene as an image asset.
  *
  * Capture is processed by the host platform layer (e.g. the Tyrian Indigo extension), which copies the last rendered
  * frame into an image, registers it with the asset system, and responds with a `Captured` event.
  */
enum ScreenCaptureEvent extends GlobalEvent:
  ...
  /** Request a screen capture.
    *
    * @param config
    *   How the capture should be produced (name, crop, scale, image type).
    * @param key
