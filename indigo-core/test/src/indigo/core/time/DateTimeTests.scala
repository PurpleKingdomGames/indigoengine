...
class DateTimeTests extends munit.FunSuite {
  ...
  test("DateTime should decompose a time of day") {
    val dt = DateTime(1700039223123L, 0) // 2023-11-15 09:07:03.123

    assertEquals((dt.year, dt.month, dt.day), (2023, 11, 15))
    assertEquals((dt.hour, dt.minute, dt.second), (9, 7, 3))
    assertEquals(dt.millisecond, 123)
  }

  test("DateTime should handle the last second of a day") {
    val dt = DateTime(86399000L, 0)
  ...
  test("DateTime should decompose dates well before the epoch") {
    val dt = DateTime(-2208988800000L, 0)
  ...
  test("DateTime should treat a year divisible by 100 but not 400 as a common year") {
    val dt = DateTime(4107542400000L, 0)
  ...
  test("DateTime should calculate the day of the week") {
    ...
    assertEquals(DateTime(4107542400000L, 0).dayOfWeek, 1) // Monday 2100-03-01
  }
  ...
  test("equality is field-wise, so the same instant at different offsets is not equal") {
    assert(DateTime(0L, 0) != DateTime(3600000L, 3600))
    assertEquals(DateTime(0L, 0).asUtc, DateTime(3600000L, 3600).asUtc)
  }
