...
object Action:
  ...
  private[tyrian] case object None extends Action:
  ...
  private[tyrian] final case class SideEffect[A](task: IO[A]) extends Action:
  ...
  private[tyrian] final case class Emit(msg: GlobalMsg) extends Action:
  ...
  private[tyrian] final case class Run[A](
      task: IO[A],
      toMsg: A => GlobalMsg
  ) extends Action:
