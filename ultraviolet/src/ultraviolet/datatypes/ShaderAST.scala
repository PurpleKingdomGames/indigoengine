  ...
  extension (ast: ShaderAST)
        ...
        remaining match
          ...
          case x :: xs =>
            x match
              ...
              case CallFunction(_, args, _)         => rec(args ++ xs, acc)
              case CallExternalFunction(_, args, _) => rec(args ++ xs, acc)
              case FunctionRef(_, args, _)          => rec(args ++ xs, acc)
    ...
    def traverse(modify: PartialFunction[ShaderAST, ShaderAST]): ShaderAST =
      ...
      ast match
        ...
        case CallFunction(id, args, rt)              => f(CallFunction(id, args.map(_.traverse(f)), rt))
        case CallExternalFunction(id, args, rt)      => f(CallExternalFunction(id, args.map(_.traverse(f)), rt))
        case FunctionRef(id, args, rt)               => f(FunctionRef(id, args.map(_.traverse(f)), rt))
